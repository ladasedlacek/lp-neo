...
form_parts.forEach(element => {
    element.addEventListener('keyup', function() {
        ...
        if (element == form_gdpr) {
            ...
            element.checkValidity() === true ? (error_message.classList.remove(error_class), form.checkValidity() === true ? button_allowed() : button_disabled()) : show_error()
        } else {
...
// form reset
const reset_form = () => {
    document.querySelector('.lpForm__statusMessage--active .body-2--link').addEventListener('click', function(event) {
        event.preventDefault() 
        form.reset()
        document.querySelector('.lpForm__statusMessage--active').classList.remove('lpForm__statusMessage--active')
